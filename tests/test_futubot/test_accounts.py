...
def test_get_market_state():
    accounts = Accounts(host='127.0.0.1',
                        port=11111,
...
def test_get_historical_candles():
    ...
    with pytest.raises(TypeError):
        accounts.get_historical_candles(code=700,
                                        start='2022-08-08 09:30:00',
                                        end='2022-08-08 10:30:00')
    with pytest.raises(TypeError):
        accounts.get_historical_candles(code='HK.00700',
                                        start=120000,
                                        end='2022-08-08 10:30:00')
    with pytest.raises(TypeError):
        accounts.get_historical_candles(code='HK.00700',
...
def test_get_positions():
    ...
    with pytest.raises(TypeError):
        accounts.get_positions(code=700)
    ...
    for key in [
            'code', 'position_side', 'stock_name', 'qty', 'cost_price',
            'market_value', 'current_price', 'pnl_ratio', 'pnl_value'
    ]:
...
def test_unlock_trade():
    ...
    with pytest.raises(TypeError):
        accounts.unlock_trade(password=123456)
    with pytest.raises(TypeError):
        accounts.unlock_trade(password='******', is_unlock='True')

    unlock_state = accounts.unlock_trade(password='******', is_unlock=True)
    assert unlock_state is False
...
def test_place_order():
    ...
    with pytest.raises(TypeError):
        accounts.place_order(price='100.0',
                             qty=100.0,
                             code='HK.00700',
    ...
    order_info = accounts.place_order(price=100.0,
                                      qty=100.0,
                                      code='HK.00700',
                                      trd_side=TrdSide.BUY,
                                      order_type=OrderType.NORMAL)
...
def test_get_lot_size():
    ...
    with pytest.raises(TypeError):
        accounts.get_lot_size(code_list='HK.00700',
                              market=Market.HK,
    ...
    lot_sizes = accounts.get_lot_size(code_list=['HK.00700'])
...
def test_check_today_orders():
    ...
    with pytest.raises(TypeError):
        accounts.check_today_orders(code=700, )
    with pytest.raises(TypeError):
        accounts.check_today_orders(code=['HK.00700'],
                                    status_filter_list='SUBMITTED')

    today_orders = accounts.check_today_orders(code='HK.00700')
...
def test_check_existing_orders():
    ...
    with pytest.raises(TypeError):
        accounts.check_existing_orders(code_list=700, )

    code_list = ['HK.00700']
    existing_orders = accounts.check_existing_orders(code_list=code_list)
...
def test_get_max_power():
    ...
    with pytest.raises(TypeError):
        accounts.get_max_power(code=700,
                               price=350.0,
                               order_type=OrderType.NORMAL)
    with pytest.raises(TypeError):
        accounts.get_max_power(code='HK.00700',
                               price='350.0',
                               order_type=OrderType.NORMAL)
    with pytest.raises(TypeError):
        accounts.get_max_power(code='HK.00700', price=350.0, order_type=1)

    code = 'HK.00700'
    price = 350.0

    max_power = accounts.get_max_power(code=code, price=price)
    assert isinstance(max_power, dict)

    for key in [
            'max_cash_buy', 'max_cash_and_margin_buy', 'max_position_sell',
            'max_sell_short', 'max_buy_back'
    ]:
